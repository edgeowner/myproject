...
public class TaskRegisterService {
    ...
    public void registerSecheduledTask(SecheduledRegisterDTO registerDTO) {
        ...
        try {
            String[] strs = registerDTO.getHostAndPort().split(":");
            if(!CollectionUtils.isEmpty(blackIPRepository.findByServiceNameAndHostAndPort(
                    registerDTO.getServiceName(), strs[0], strs[1]))) {
                throw new RuntimeException("黑名单服务不能注册");
            }
     ...
     * @param requestDTO
     */
    public void cancelDelayTask(DelayRequestDTO requestDTO) {
        if(requestDTO == null) {
            return;
        }
        List<DelayTaskInfo> list = delayTaskInfoRepository.findByBizNameAndBizParameters(requestDTO.getBizName(), requestDTO.getBizParameters());
        for(DelayTaskInfo taskInfo : list) {
            if(DelayTaskInfo.WAITING.equals(taskInfo.getStatus())) {
                taskInfo.setStatus(DelayTaskInfo.CANCEL);
                taskInfo.setRemark("业务系统请求取消");
                delayTaskInfoRepository.save(taskInfo);
