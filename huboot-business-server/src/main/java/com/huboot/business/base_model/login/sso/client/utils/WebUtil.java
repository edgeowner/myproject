...
public class WebUtil {
	 ...
	 * **/
	public static <T> T readObjectFromReq(HttpServletRequest request,Class<T> c) throws IOException, IllegalAccessException, IllegalArgumentException, InstantiationException{
		ServletInputStream sis = request.getInputStream();
		if(request.getContentLength() == 0) throw new IOException("请求内容为空");
		byte[] b = new byte[request.getContentLength()];
		sis.read(b, 0, b.length);
		String content = new String(b, StandardCharsets.UTF_8.name());
		String jsonString=jsonStringRelace(content);
		return new Jackson2ObjectMapperBuilder().build().readValue(jsonString, c);
	}
	 ...
	 * @throws IllegalAccessException 
	 * **/
	public static byte[] readBytesFromReq(HttpServletRequest request) throws IOException, IllegalAccessException, IllegalArgumentException, InstantiationException{
		if(request == null ) throw new IllegalAccessException("参数request不能为空");
		if(request.getContentLength() == 0) return new byte[0];
		ServletInputStream sis = request.getInputStream();
		byte[] b = new byte[request.getContentLength()];
		sis.read(b, 0, b.length);
		return b;
	...
	public static void main(String[] args) throws JsonParseException, JsonMappingException, IOException, IllegalAccessException {
		...
		Map<String,String> map2 = beanToMapWithString(loginDTO, true);
		System.out.println(map2.get("account"));
		List<LoginDTO> list = new ArrayList<>();
		list.add(loginDTO);
		list.add(loginDTO);
		Map<String,String> map3 = beanToMapWithString(list, true);
		System.out.println(123);
		System.out.println("----------------------");
		//System.out.println(genSign(loginDTO,"abc123"));
		List map4 = mapper.readValue(mapper.writeValueAsString(list), List.class);
	...
	public static String genQueryStr(Object obj,String key,String sysName) throws IllegalAccessException, UnsupportedEncodingException {
		...
		buffer.append(ApiAuthenticationTokenFilter.API_KEY_SIGN);
		buffer.append("=");
		buffer.append(sign);
		buffer.append("&");
		buffer.append(ApiAuthenticationTokenFilter.API_KEY_SYS);
		buffer.append("=");
	...
	public static String genSign(Object obj,String key) throws IllegalAccessException, UnsupportedEncodingException {
		...
		if(obj == null) {
			...
			map.put(ApiAuthenticationTokenFilter.API_MAP_SIZE, "0");
			map.put(ApiAuthenticationTokenFilter.API_MAP_TYPE, ApiAuthenticationTokenFilter.API_REQ_TYPE_VOID);
			map.put(ApiAuthenticationTokenFilter.API_MAP_TIME, LocalDateTime.now().format(DATE_YYYY_MM_DD_HH_MM_SS));
			return sign(map,key);
		}
	 ...
	 * @param obj
	 * @param keepNull
	 *            如果keepNull=ture,则返回的obj中的空属性默认为'',否则将被过滤
	 * @throws IllegalAccessException 
	 **/
	public static Map<String, String> beanToMapWithString(Object obj, boolean keepNull) throws IllegalAccessException {
		...
		if (obj instanceof Collection) {
			...
			map.put(ApiAuthenticationTokenFilter.API_MAP_SIZE, collections.size()+"");
			map.put(ApiAuthenticationTokenFilter.API_MAP_TYPE, ApiAuthenticationTokenFilter.API_REQ_TYPE_COLLECTION);
			return map;
		};
		//map 处理
		if(obj instanceof Map) {
			...
			if(map.keySet() != null && map.keySet().size() == 0) {
				map.put(ApiAuthenticationTokenFilter.API_MAP_SIZE, "0");
				map.put(ApiAuthenticationTokenFilter.API_MAP_TYPE, ApiAuthenticationTokenFilter.API_REQ_TYPE_MAP);
				return map;
			}
			return map;
		}
		...
		if(obj instanceof Number) {
			map.put(ApiAuthenticationTokenFilter.API_MAP_SIZE, 1+"");
			map.put(ApiAuthenticationTokenFilter.API_MAP_TYPE, ApiAuthenticationTokenFilter.API_REQ_TYPE_NUMBER);
		}
		if(obj instanceof String) {
			map.put(ApiAuthenticationTokenFilter.API_MAP_SIZE, 1+"");
			map.put(ApiAuthenticationTokenFilter.API_MAP_TYPE, ApiAuthenticationTokenFilter.API_REQ_TYPE_STRING);
		}
		//其他对象
		try {
