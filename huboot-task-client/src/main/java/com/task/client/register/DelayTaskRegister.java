...
public class DelayTaskRegister {
     ...
     * @return
     */
    public String register(String bizName, String bizParameters, Date executeTime) {
        DelayTaskData taskData = new DelayTaskData();
        taskData.setBizName(bizName);
        taskData.setBizParameters(bizParameters);
        taskData.setExecuteTime(executeTime);
        return this.register(taskData);
    }
     ...
     * @param bizParameters
     */
    public void cancel(String bizName, String bizParameters) {
        if(StringUtils.isEmpty(bizName)) {
            throw new TaskClientException("取消延迟任务bizName不能为空");
        }
        if(StringUtils.isEmpty(bizParameters)) {
            throw new TaskClientException("取消延迟任务bizParameters不能为空");
        }
        Map<String, String> map = new HashMap<>();
        map.put("bizName", bizName);
        map.put("bizParameters", bizParameters);
        requestHelper.sendRequest(ServerURL.DELAY_TASK_CANCEL_PARAMETERS_URL, map);
    }
